...
int main(int argc, char **argv) { 
	...
	hashtable ht(29587); // Randomly large prime hash number.
	string line;
	...
	int numWords = 0;
	for (unsigned int r = 0; r < rows; r++) { // Going through the rows.
		for (unsigned int c = 0; c < cols; c++) { // Going through the columns.
			for (unsigned int d = 0; d < 8; d++) { // Going through each direction.
				for (unsigned int l = 3; l <= 22; l++) { // Going through each length of word(s).
    ...
    // cout << std::to_string(t.getTime() * 1000) << endl; // Total milliseconds taken.
